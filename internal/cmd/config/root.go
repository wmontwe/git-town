...
func printConfig(config config.UnvalidatedConfig, showSecrets configdomain.ShowSecrets) {
	...
	print.Entry("fork stack", format.Bool(bool(config.NormalConfig.ForkStack)))
	print.Entry("fork stack label", format.OptionalStringerSetting(config.NormalConfig.ForkStackLabel))
	fmt.Println()
	print.Header("Ship")
